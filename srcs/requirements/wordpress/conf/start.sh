...
if [ ! -f "$lock_file" ]; then
	...
	wp core install --allow-root --url="${FDQN}" --title="Biopic de Christophe ASSASSA" --admin_user="${WP_ADMIN_USR}" --admin_password="${WP_ADMIN_PASSW}" --admin_email="${WP_ADMIN_EMAIL}" --path="/var/www/html" --skip-email && \
